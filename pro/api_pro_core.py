...
class DownloadApplicantResumeAPIView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        html = self.template.render({'data': queryset, 'SITE_URL': settings.SITE_URL,'STATIC_URL': settings.STATIC_URL,
                                     'MEDIA_BUCKET_URL_PREFIX': settings.MEDIA_BUCKET_URL_PREFIX, 'STATIC_BASE_URL_PREFIX': settings.STATIC_BASE_URL_PREFIX})
        options = {
            'page-size': "A4",
            'encoding': "UTF-8",
...
class VerifyMobileVerificationCodeVOnePointZeroPointSixtyFour(GenericAPIView, UpdateModelMixin):
    permission_classes = [ProfessionalPermission]
    queryset = Professional.objects.all()
    serializer_class = ProfessionalSerializer
    current_user = None
