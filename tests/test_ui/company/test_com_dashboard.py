...
class TestCompanyInfoBox(unittest.TestCase):
    ...
    def test__when_company_notifications_page_is_correct__pass(self):
        ...
        notifications_url = driver.current_url
        time.sleep(1)
        side_menu_notifications = driver.find_element_by_css_selector(".company_notification.active").text
        driver.execute_script("window.history.go(-1)")
        time.sleep(DELAY_SHORT)
        side_menu_dashboard = driver.find_element_by_css_selector(".dashboard.active").text
        if notifications_url != NOTIFICATIONS_URL:
            self.fail()
        if side_menu_notifications != SIDE_MENU_NOTIFICATIONS_NAME:
            self.fail()
        if side_menu_dashboard != SIDE_MENU_DASHBOARD_NAME:
            self.fail()
