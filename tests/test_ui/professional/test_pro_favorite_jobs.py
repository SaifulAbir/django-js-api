...
from tests.test_ui.company.test_signin import signout_helper
            ...
            try:
                if total_favorite_jobs_count_show != TOTAL_FAVORITE_JOBS_COUNT_MINUS_1 and total_favorite_jobs_count_show != TOTAL_FAVORITE_JOBS_COUNT_10_MINUS_1 and total_favorite_jobs_count_show != TOTAL_FAVORITE_JOBS_COUNT_11_MINUS_1:
                    try:
                        ...
                        time.sleep(DELAY_SHORT)
                        success_favorite = driver.find_element_by_xpath('//*[@id="swal2-content"]').text
                        time.sleep(1)
...
class TestFavoritedJobCompanyDetail(unittest.TestCase):
    ...
    def test__company_detail_from__professional_favorite_jobs__showing_data__should_pss(self):
        driver = self.driver
        driver.get(MAIN_URL_HOME)
        COMPANY_URL = f'{MAIN_URL_HOME}/company-details/Company%20Example%20View'
        COMPANY_NAME = 'Company Example View'
        JOB_FAVORITE_LIST_URL = f'{MAIN_URL_HOME}/professional/favorite-jobs/'
        data = {
            "email": VALID_PRO_VIEW_USERNAME,
            "password": VALID_PRO_VIEW_PASSWORD,
        }
        signin_helper(driver, data)
        job_list = job_list_with_signin_helper(driver, JOB_FAVORITE_LIST_URL)
