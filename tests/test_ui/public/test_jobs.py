...
from tests.test_ui.company.test_signin import signout_helper, signin_helper_company
from tests.test_ui.professional.test_signin import signin_helper
from tests.test_ui.public.test_company_detail import job_list_with_signin_helper, company_detail_helper, \
    job_list_without_signin_helper
...
most_applied_job_result_show_10_for_fresh_db = ['Job Post Example Five', 'Job Post Example Three', 'Job Post Example Twenty', 'Job Post Example Four', 'Job Post Example Fourteen', 'Job Post Example Eleven', 'Job Post Example Fourteen Double', 'Job Post Example Twenty One', 'Job Post Example Seventeen', 'Job Post Example Company Two']
...
class TestJobSearchCompanyDetail(unittest.TestCase):
    ...
    def test_job_search_page__company_detail_showing_data__when_professional_signed_in__should_pss(self):
        driver = self.driver
        driver.get(MAIN_URL_HOME)
        COMPANY_URL = f'{MAIN_URL_HOME}/company-details/Company%20Example%20View'
        COMPANY_NAME = 'Company Example View'
        job_list_url = f'{MAIN_URL_HOME}/jobs/'
        data = {
            "email": VALID_PRO_USERNAME,
            "password": VALID_PRO_PASSWORD,
        }
        signin_helper(driver, data)
        job_list = job_list_with_signin_helper(driver, job_list_url)
        signout_helper(driver)
        for url in job_list['company_url_list']:
            if url == COMPANY_URL:
                company_detail = company_detail_helper(driver, url)
                break
        self.assertEqual(company_detail['company_name'], COMPANY_NAME)
        time.sleep(1)


    def test_job_search_page__company_detail_showing_data__when_company_signed_in__should_pss(self):
        driver = self.driver
        driver.get(MAIN_URL_HOME)
        COMPANY_URL = f'{MAIN_URL_HOME}/company-details/Company%20Example%20View'
        COMPANY_NAME = 'Company Example View'
    ...
    def test_job_search_page__company_detail_showing_data__without_signed_in__should_pss(self):
        driver = self.driver
        driver.get(MAIN_URL_HOME)
        COMPANY_URL = f'{MAIN_URL_HOME}/company-details/Company%20Example%20View'
        COMPANY_NAME = 'Company Example View'
        job_list_url = f'{MAIN_URL_HOME}/jobs/'
        job_list = job_list_without_signin_helper(driver, job_list_url)
        for url in job_list['company_url_list']:
            if url == COMPANY_URL:
                company_detail = company_detail_helper(driver, url)
                break
...
class TestFilterSearchResultCount(unittest.TestCase):
    ...
    def test_filter_search__when_not_signed_in__job_search_most_applied_job__should_pss(self):
        ...
        try:
            ...
            if titles != most_applied_job_result_show_10 and titles != most_applied_job_result_show_10_a and titles != most_applied_job_result_show_10_a_pro and titles != most_applied_job_result_show_10_a_pro_b and titles != most_applied_job_result_show_10_a_pro_c and titles != most_applied_job_result_show_10_for_fresh_db:
                self.fail()
        except NoSuchElementException:
            self.fail('Not ok')
