...
class TestCompanyDetail(unittest.TestCase):
    ...
    def test_job_detail__when__company_signed_in__view_recent_jobs__should_pss(self):
        ...
        if job_list['job_title_list'] != COMPANY_EXAMPLE_VIEW_RECENT_JOBS_WITH_SIGNIN:
            self.fail()
        time.sleep(1)
    ...
    def test_job_detail__without_signed_in__view_recent_jobs__should_pss(self):
        ...
        if job_list['job_title_list'] != COMPANY_EXAMPLE_VIEW_RECENT_JOBS_WITHOUT_SIGNIN:
            self.fail()
        time.sleep(1)
    ...
    try:
        ...
        for i in range(i, page_count - 2):
            ...
            for job in job_list:
                ...
                try:
                    title = job.find_element_by_tag_name('h4')
                    job_title = title.find_element_by_tag_name('a').text
                    job_url = title.find_element_by_tag_name('a').get_attribute('href')
                    try:
                        company = job.find_element_by_class_name('company')
                        company_url = company.find_element_by_tag_name('a').get_attribute('href')
                    except:
                        company_url = ''
                except:
                    job_title = ''
            ...
            time.sleep(DELAY_SHORT)
        job_list_data = {
            'job_title_list' : titles,
            'job_url_list' : job_url_list,
            'company_url_list' : company_url_list
        }
